...
export function ResultsPane({
  ...
  const renderHighlightedText = () => {
    ...
    for (const correction of sorted) {
      ...
      if (offset > cursor) {
        segments.push(<span key={`plain-${cursor}`}>{text.slice(cursor, offset)}</span>);
      }
        ...
        <span
          ...
          onClick={(e) => {
            ...
            if (isOpen) {
              ...
              setPopover({ offset, x: rect.left, y: rect.bottom + 4 });
            }
          }}
          style={{
  ...
  const activeCorrection = corrections.find((c) => c.offset === popover?.offset);

  return (
    <div className="flex flex-col gap-3 h-full" style={{ fontFamily: "'Share Tech Mono', monospace" }}>

      {/* Output pane */}
      <div ref={containerRef} className="cyber-border bg-card p-4 flex-1 relative overflow-visible">
        <span className="absolute top-0 left-0 w-2 h-2 border-t border-l border-primary" />
        <span className="absolute top-0 right-0 w-2 h-2 border-t border-r border-primary" />
        <span className="absolute bottom-0 left-0 w-2 h-2 border-b border-l border-primary" />
        <span className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-primary" />
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xs tracking-widest uppercase text-primary glow-text">// Output</h2>
        ...
        {/* Always show text if we have it */}
        {text && (
          <div
            className="text-sm leading-relaxed text-foreground whitespace-pre-wrap break-words"
            style={{ fontFamily: "'Share Tech Mono', monospace" }}
          >
            {corrections.length > 0 ? renderHighlightedText() : text}
          </div>
        )}

        {/* Only show no errors state if backend returned 0 errors and no text */}
        {!text && stats.errors === 0 && (
          <div className="flex flex-col items-center justify-center gap-3 py-8 text-center">
            ...
            <p className="text-sm tracking-widest uppercase text-green-500">No Errors Detected</p>
            <p className="text-xs text-muted-foreground tracking-wider">All systems nominal.</p>
          </div>
        )}

        {/* Popover — fixed position so it's never clipped */}
        {popover && activeCorrection && (
          <div
            style={{ position: "fixed", left: popover.x, top: popover.y, zIndex: 9999, minWidth: "160px" }}
            className="cyber-border bg-card border border-primary shadow-lg"
          >
            {activeCorrection.suggestions.length > 0 ? (
              <div>
                <p className="text-xs tracking-widest uppercase text-muted-foreground px-3 pt-2 pb-1">Suggestions</p>
                {activeCorrection.suggestions.slice(0, 5).map((s, i) => (
                  <button
                    key={i}
                    onClick={() => { onApplyCorrection(activeCorrection.offset, s.word); setPopover(null); }}
              ...
              <p className="text-xs text-muted-foreground px-3 py-2">No suggestions</p>
            )}
            <div className="border-t border-border">
              <button
                onClick={() => { onIgnore(activeCorrection.offset); setPopover(null); }}
      ...
      {corrections.length > 0 && (
        <div className="cyber-border bg-card border border-primary/40 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between px-3 py-2 border-b border-border">
            <p className="text-xs tracking-widest uppercase text-primary">// Errors ({corrections.length})</p>
            <button
              onClick={onApplyAll}
              className="text-xs tracking-widest uppercase px-3 py-1 border border-primary text-primary hover:bg-primary hover:text-primary-foreground transition-all duration-200 glow"
            >
          ...
          <div className="max-h-32 overflow-y-auto">
            {corrections.map((c) => (
              <div key={c.offset} className="flex items-center justify-between px-3 py-1.5 border-b border-border/50 last:border-b-0 hover:bg-muted/20">
                <div className="flex items-center gap-2">
                  <span className="text-xs text-destructive font-bold">{c.original}</span>
                  {c.suggestions.length > 0 && (
                    <>
                      <span className="text-xs text-muted-foreground">→</span>
                      <span className="text-xs text-primary">{c.suggestions[0].word}</span>
                    </>
                  )}
                </div>
                <div className="flex gap-2">
                  {c.suggestions.length > 0 && (
                    <button
                      onClick={() => onApplyCorrection(c.offset, c.suggestions[0].word)}
